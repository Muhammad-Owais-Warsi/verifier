...
Tasks are packaged as [Harbor](https://www.harborframework.com) tasks, so anyone can run them with only Docker installed:

| Path | Brief |
| --- | --- |
| `harbor/video-encoder/` | `tasks/video-transcode` — transcode uploads into several formats |
| `harbor/driving-license/` | `tasks/driving-license` — driving licence portal backend |
| `harbor/image-process/` | `tasks/image-process` — thumbnails, optimized versions and formats from an upload |

```bash
harbor run --path harbor/driving-license --agent <agent> --model <provider>/<model>
```
...
Each task folder is generated by `npm run harbor:sync` from `verifier/`, its own `tasks/<name>/` and the shared `tasks/setup.md`. The originals stay the source of truth, so the container grades with exactly the verifier you ran locally — re-run it after changing any of them. Adding a task is one `sync` line in `harbor/sync.sh`, plus the four files that differ only by task name: `task.toml`, `instruction.md`, `environment/Dockerfile` and `tests/test.sh`.
...
Task name is the folder under `tasks/` (`driving-license`, `video-transcode`, ...). Or point at any expectations file directly:
...
```bash
npm run fixtures   # expect: 199/199 assertions passed across 28 fixtures
```
